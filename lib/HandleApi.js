...
process.on('unhandledRejection', (reason, p) => {
	...
	let code = 400
	HandleError(err, code, p.toString()+"::"+reason.toString())
 });
	...
	this.deleteHashData = function( collection, hashData, owner = false ) {
		return new Promise( ( resolve, reject ) => {
			this.sto.findByHash( hashData, collection, owner ).then( (data) => {
				...
				if( this.cache.get( cache_key_count ) ) {
					this.cache.del( cache_key_count ) // could decrement will delete for now
				}

				this.sto.deleteHashData( hashData, collection, owner ).then( ( msg ) => {
					this.cache.del( cache_key_hash ) // could return error code 410 -- 
					resolve( msg )
				}).catch( ( err ) => {
					reject( err )
				}) 	
			...
			this.readOwner( collection, owner, limit, api_key, page, sort, init ).then((dataset) => {
				...
				new Promise( ( res, rej ) => {
					dataset.map((data) => {
						...
						this.deleteHashData(collection, data.hashData, owner ).then( () => {
							res(true)
						} ).catch( (err) => {
							rej(err)
						})
