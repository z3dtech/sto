...
module.exports = function( ) {
	...
	this.buildFile = function( config_input ) {
		return new Promise( ( res, rej ) => {
			jsonfile.readFile( __dirname + '/../config.example.json', function( err, config ) { 
				if( err ) {
					rej( HandleError( '[CONFIG] Something went wrong.', true ) )
				} else {
					...
					jsonfile.writeFile( configfile, config, {spaces: 2, EOL: '\r\n'}, function(err) {
						if( err !== null ) {
							rej( HandleError( '[CONFIG] Error writing config file: ' + err, true ) )
						} else {
							res( true )
						}
					})			
		...
		let setup = [
			{
				...
				validate: function( value ) {
					if( !value.match(/^(mongodb:(?:\/{2})?)((\w+?):(\w+?)@|:?@?)([A-Za-z0-9-_.]+?):(\d+)\/([A-Za-z0-9-_]+?)$/) ) {
